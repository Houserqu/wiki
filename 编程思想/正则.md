...
- [正则表达式 (regex) 的快速参考](https://www.w3cschool.cn/regexp/regexp-dgbp3khn.html)
...
是一种零宽度的匹配，即它匹配到的内容不会保存到匹配结果中去，常用于获取不包含边界的内容

```js
// 示例：只获取 url 中的域名
"https://www.baidu.com/login/name".match(/(?<=https?:\/\/)[\w\.]*(?=\/)/)
// 输出：[''www.baidu.com', index: 8]
// (?<=https?:\/\/) 开始边界，格式：(?<=正则)
// (?=\/) 结束边界，格式 (?=正则)
```
...
| RegExp   | [test (en-US)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/test) |
| String   | [match (en-US)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/match) |
| String   | [replace](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/String/replace) |
| String   | [search (en-US)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/search) |
| String   | [split (en-US)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/split) |
